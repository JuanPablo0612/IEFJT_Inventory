...
import com.juanpablo0612.iefjt.domain.usecase.SaveBrandUseCase
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class AddBrandViewModel @Inject constructor(private val saveBrandUseCase: SaveBrandUseCase) :
    ViewModel() {
    var uiState by mutableStateOf(AddBrandUiState())
        private set
    ...
    fun onSaveBrand() {
        viewModelScope.launch {
            ...
            if (uiState.nameValid && uiState.imageUrlValid && uiState.imageUriValid) {
                ...
                saveBrandUseCase(uiState.name.trim(), uiState.imageUrl.trim(), uiState.imageUri).fold(
                    onSuccess = {
